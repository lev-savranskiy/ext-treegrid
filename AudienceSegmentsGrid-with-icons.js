...
App.AudienceSegmentsUtils = {
    ...
    createButton: function(action, obj) {
        ...
        return String.format('<img style="margin:  1px 2px;" src="icons/{0}.png" onclick="App.AudienceSegmentsCalls.{0}(\'{1}\');"  qtip="{0}">', action, obj);
    ...
    createButtonDisabled: function(action) {
        ...
        return String.format('<img style="margin:  1px 2px;" src="icons/{0}-disabled.png" qtip="Disabled for current segment status">', action);
...
//    addsec: function(id){
//
//         debug(App.AudienceSegmentsUtils.getId(id));
//
//        //alert(App.AudienceSegmentsUtils.getId(id));
//    },
        ...
        Ext.MessageBox.show({
            ...
            fn: function(buttonId) {
                ...
                Ext.Ajax.request({
                    ...
                    success: function(response, request) {
                        window.location.href = '?afterunpublish';
                        App.AudienceSegmentsUtils.unmask(win);
                    },
                    failure: function() {
                        App.AudienceSegmentsUtils.unmask(win);
...
App.SplitButton =  function(objid) {
...
var id = objid.replace('SplitButtonClicker' , '')
    ...
    // toggle if exists
      if (!! Ext.getCmp('SplitButton' + id)){
          Ext.getCmp('SplitButton' + id).destroy();
          return false;
      }
 debug(id);
                ...
                items: [
                    {
                        text: 'Publish',
                        iconCls: 'publish',
                        handler: function(){return App.AudienceSegmentsCalls['publish'](id)}
                    },
                    {
                        text: 'Unpublish',
                        iconCls: 'unpublish',
                        handler: function(){return App.AudienceSegmentsCalls['unpublish'](id)}
                    },
                    {
                        text: 'Delete',
                        iconCls: 'delete',
                        handler: function(){return App.AudienceSegmentsCalls['delete'](id)}
                    }
                ]
            }
        });
...
//            },
    ...
    listeners: {
        contextmenu: function(node, e) {
            ...
            if (selId.strpos('seg') === 0) {

                if (selStatus == 'Unpublished') {


                    c.addItem({
                        iconCls: 'publish',
                        id: 'publish-segment',
                        text: 'Publish segment'
                    });
                ...
                if (selStatus == 'Published') {
                    ...
                    c.addItem({
                        iconCls: 'unpublish',
                        id: 'unpublish-segment',
                        text: 'Unpublish segment'
                    });
            ...
            if (selId.strpos('sec') === 0) {

                if (node.parentNode.attributes.status == 'Unpublished') {
                    c.addItem({
                        iconCls: 'edit',
                        id: 'edit-section',
                        text: 'Edit section'
                    ...
                    c.addItem({
                        iconCls: 'error',
                        id: 'info',
                        text: 'You can`t manage sections in ' + node.parentNode.attributes.status + ' Segments'
                    });
                }
...
//        }
        ...
        Ext.Ajax.request({
            ...
            url:  App.AudienceSegmentsUtils.getActionUrl('copySegment', {
                'segment-id': this.selNode.attributes.tid
            }),
            ...
            Ext.Ajax.request({
                ...
                success: (function(form) {
                    return function(response, request) {
                        ...
                        App.AudienceSegmentsUtils.getRoot().appendChild(params);
